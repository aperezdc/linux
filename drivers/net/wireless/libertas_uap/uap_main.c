...
uap_get_mac_address(uap_private * priv)
{
	...
	CmdSize = S_DS_GEN + sizeof(HostCmd_SYS_CONFIG) + sizeof(MrvlIEtypes_MacAddr_t);
	cmd = (HostCmd_DS_COMMAND *) (skb->data + INTF_HEADER_LEN);
	...
	pMacAddrTlv = (MrvlIEtypes_MacAddr_t *) (skb->data + INTF_HEADER_LEN + S_DS_GEN +
	...
	pTlv = (MrvlIEtypes_Data_t *) (Adapter->CmdBuf + S_DS_GEN +
				       sizeof(HostCmd_SYS_CONFIG));
...
uap_init_sw(uap_private *priv)
{
	...
	if (!(Adapter->nl_sk = netlink_kernel_create(&init_net, NETLINK_MARVELL, NULL))) {
		PRINTM(ERROR, "Could not initialize netlink event passing mechanism!\n");
	}
	LEAVE();
...
uap_hostcmd_ioctl(struct net_device *dev, struct ifreq *req)
{
	...
	if (copy_from_user(&head, req->ifr_data + sizeof(buf_len), sizeof(HostCmd_HEADER))) {
		PRINTM(ERROR, "Copy from user failed\n");
		LEAVE();
		return -EFAULT;
	}
	...
	if (copy_from_user(skb->data + INTF_HEADER_LEN, req->ifr_data + sizeof(buf_len),
	     head.Size)) {
		PRINTM(ERROR, "Copy from user failed\n");
		LEAVE();
		return -EFAULT;
	...
	if (UAP_STATUS_SUCCESS != uap_process_cmd(priv, skb, HostCmd_OPTION_WAITFORRSP)) {
	...
	/* Copy to user */
	if (copy_to_user(req->ifr_data + sizeof(buf_len), Adapter->CmdBuf, Adapter->CmdSize)) {
...
uap_power_mode_ioctl(struct net_device *dev, struct ifreq *req)
{
	...
	if (pm_cfg.flags & ~(PS_FLAG_PS_MODE | PS_FLAG_SLEEP_PARAM | PS_FLAG_INACT_SLEEP_PARAM)) {
	...
	if (!pm_cfg.flags) {
		...
		if ((pm_cfg.ps_mode) && (pm_cfg.flags & PS_FLAG_SLEEP_PARAM)) {
			...
			sleep_tlv->header.Len = uap_cpu_to_le16(sizeof(MrvlIEtypes_sleep_param_t) -
								sizeof(MrvlIEtypesHeader_t));
			sleep_tlv->ctrl_bitmap = uap_cpu_to_le32(pm_cfg.sleep_param.ctrl_bitmap);
			sleep_tlv->min_sleep = uap_cpu_to_le32(pm_cfg.sleep_param.min_sleep);
			sleep_tlv->max_sleep = uap_cpu_to_le32(pm_cfg.sleep_param.max_sleep);
		...
		if ((pm_cfg.ps_mode == PS_MODE_INACTIVITY) &&
			...
			inact_tlv->header.Type = uap_cpu_to_le16(TLV_TYPE_AP_INACT_SLEEP_PARAM);
			inact_tlv->header.Len = uap_cpu_to_le16(sizeof(MrvlIEtypes_inact_sleep_param_t) -
								sizeof(MrvlIEtypesHeader_t));
			inact_tlv->inactivity_to = uap_cpu_to_le32(pm_cfg.inact_param.inactivity_to);
			inact_tlv->min_awake = uap_cpu_to_le32(pm_cfg.inact_param.min_awake);
			inact_tlv->max_awake = uap_cpu_to_le32(pm_cfg.inact_param.max_awake);
	...
	if (pm_cfg.flags) {
		...
		tlv_buf_left = cmd->Size - (sizeof(HostCmd_DS_POWER_MGMT_EXT) + S_DS_GEN);
		tlvbuf = (MrvlIEtypesHeader_t *) ((u8 *) & cmd->params.pm_cfg +
		...
		while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
			...
			switch (tlv_type) {
				case TLV_TYPE_AP_SLEEP_PARAM:
					...
					pm_cfg.sleep_param.ctrl_bitmap = uap_le32_to_cpu(sleep_tlv->ctrl_bitmap);
					pm_cfg.sleep_param.min_sleep = uap_le32_to_cpu(sleep_tlv->min_sleep);
					pm_cfg.sleep_param.max_sleep = uap_le32_to_cpu(sleep_tlv->max_sleep);
					break;
				case TLV_TYPE_AP_INACT_SLEEP_PARAM:
					inact_tlv = (MrvlIEtypes_inact_sleep_param_t *) tlvbuf;
					pm_cfg.flags |= PS_FLAG_INACT_SLEEP_PARAM;
					pm_cfg.inact_param.inactivity_to = uap_le32_to_cpu(inact_tlv->inactivity_to);
					pm_cfg.inact_param.min_awake = uap_le32_to_cpu(inact_tlv->min_awake);
					pm_cfg.inact_param.max_awake = uap_le32_to_cpu(inact_tlv->max_awake);
					break;
			}
			tlv_buf_left -= tlv_len + sizeof(MrvlIEtypesHeader_t);
			tlvbuf = (MrvlIEtypesHeader_t *) ((u8 *) tlvbuf + tlv_len +
							  sizeof(MrvlIEtypesHeader_t));
		}
		/* Copy to user */
...
uap_bss_stop(uap_private *priv)
{
	...
	if (UAP_STATUS_SUCCESS != uap_process_cmd(priv, skb, HostCmd_OPTION_WAITFORRSP_TIMEOUT)) {
...
uap_soft_reset(uap_private * priv)
{
	...
	if (UAP_STATUS_SUCCESS != uap_process_cmd(priv, skb, HostCmd_OPTION_WAITFORSEND)) {
...
uap_set_mac_address(struct net_device *dev, void *addr)
{
	...
	pMacAddrTlv = (MrvlIEtypes_MacAddr_t *) ((u8 *) cmd + S_DS_GEN +
						 sizeof(HostCmd_SYS_CONFIG));
	pMacAddrTlv->Header.Type = uap_cpu_to_le16(MRVL_AP_MAC_ADDRESS_TLV_ID);
	pMacAddrTlv->Header.Len = uap_cpu_to_le16(ETH_ALEN);
...
uap_process_event(uap_private *priv, u8 *payload, uint len)
{
	...
	if ((pEvent->EventId == EVENT_PS_AWAKE) || (pEvent->EventId == EVENT_PS_SLEEP))
		goto done;
